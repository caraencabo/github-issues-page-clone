...
export default function Issues() {
    ...
    const handlePagination = (url) => {
        setCurrentPage((prevPage) => prevPage + (url === nextPageUrl ? 1 : -1));
        fetchData(url);
        const newPageNumber = url.match(/\?page=(\d+)/);
        if (newPageNumber) {
            router.push(`/issues?page=${newPageNumber[1]}`);
        }
    };
    ...
    const handleFilterChange = (selectedFilter) => {
        ...
        fetchData(newUrl);
        router.push(`/issues?page=${currentPage}&filter=${selectedFilter}`);
    };
    ...
    useEffect(() => {
        ...
        fetchData(`https://api.github.com/repos/${repoOwner}/${repoName}/issues?page=${pageNumber}`);
    }, [router.query.page]);
          ...
          <div className="flex justify-center p-4">
            {prevPageUrl && currentPage > 1 && (
              <button
                ...
                onClick={() => handlePagination(prevPageUrl)}
              >
                Previous
              </button>
            )}
            {nextPageUrl && (
              <button
                ...
                onClick={() => handlePagination(nextPageUrl)}
              >
                Next
              </button>
            )}
